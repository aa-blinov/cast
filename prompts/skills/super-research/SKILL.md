---
name: super-research
description: Autonomous long-running research (minutes to overnight) producing honest, comparable, auditable evidence instead of one-shot answers. Eight modes, picked from the request: (1) experiment loop — edit-run-measure, keep/revert ('optimize X', 'tune hyperparameters', '自动实验'); (2) topic survey / 主题调研 ('literature review', 'deep research'); (3) quantitative analysis / 量化分析 — hypothesis-first, schema audit, effect sizes; (4) benchmark comparison / 对比评测 ('compare X vs Y', '选型'); (5) root-cause investigation / 根因排查 — regressions, flakes, perf drops; (6) ablation study / 消融实验 — leave-one-out attribution vs a noise floor; (7) paper reproduction / 复现论文 — paper to working code; (8) paper writing + citation audit / 写论文 & 引用校验 — verify every citation against real APIs. Zero-external-dependency toolbox: built-in tools + free scholarly APIs (arXiv, Semantic Scholar, OpenAlex, Crossref), no API keys. Trigger for any research work with volume + discipline, even without the words 'research' or 'experiment'.
---
