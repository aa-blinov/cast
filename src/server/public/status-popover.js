...
export function StatusPopover({ activeId, running }) {
	...
	return html`<button class="menu-toggle" onClick=${openModal} aria-label="Status" title="Status"><${icons.info} /></button>
		${
			...
			<div class="modal-header"><h2 class="modal-title">Status</h2><button class="modal-close" onClick=${() => setOpen(false)} aria-label="Close"><${icons.xMark} /></button></div>
			<div class="modal-status-body">${error && html`<div class="settings-error" role="alert">${error}</div>`}${!data && !error ? html`<div class="settings-loading">Loading</div>` : html`<${SettingsStatus} data=${data} />`}</div>
		</div></div>`
		}`;
}
