	...
	return html`
		<div class="settings-rows">
			...
			<div class="settings-inline-form">
				<input aria-label="Turn iteration safety cap" type="text" inputmode="numeric" pattern="[0-9]*" value=${capDraft} disabled=${busy} onInput=${(event) => setCapDraft(event.target.value.replace(/[^0-9]/g, ""))} />
			...
			<div class="settings-compact-list">
				...
				${["checkpoint", "memory", "notes", "global", "tasks"].map((key) => {
					...
					return html`<div class="settings-compact-row" key=${`caps-${key}`}>
						...
						<form class="settings-inline-form" onSubmit=${(event) => { event.preventDefault(); actCaps(key); }}><input aria-label=${`Checkpoint ${key} token cap`} type="text" inputmode="numeric" pattern="[0-9]*" placeholder=${current} value=${capsDrafts[key] ?? ""} disabled=${busy || !enabled || !writeEnabled} onInput=${(e) => { const v=e.target.value.replace(/[^0-9]/g, ""); setCapsDrafts({...capsDrafts, [key]: v}) }} /><button class="modal-btn icon-btn" title="Reset to ${DEFAULTS.caps[key]}" disabled=${busy || !enabled || !writeEnabled || (pushCaps[key] === undefined || pushCaps[key] === DEFAULTS.caps[key])} onClick=${() => act(`/memory checkpoint caps ${key}=${DEFAULTS.caps[key]}`)}><${icons.arrowUturnLeft} /></button><button class="modal-btn icon-btn" title="Save" disabled=${busy || !capsDrafts[key]} onClick=${() => actCaps(key)}><${icons.check} /></button></form>
					</div>`;
				})}
				<div class="settings-compact-row">
				...
				<div class="settings-compact-row">
					...
					<form class="settings-inline-form" onSubmit=${(event) => { event.preventDefault(); const value = Number(distillIntervalDraft || distillInterval); if (Number.isInteger(value) && value >= 0 && value <= 3650) act(`/memory distill interval ${value}`); }}><input aria-label="Automatic distill interval days" type="text" inputmode="numeric" pattern="[0-9]*" value=${distillIntervalDraft} disabled=${busy || !enabled || !writeEnabled} onInput=${(event) => setDistillIntervalDraft(event.target.value.replace(/[^0-9]/g, ""))} /><button class="modal-btn icon-btn" title="Reset to ${DEFAULTS.distillInterval}" disabled=${busy || !enabled || !writeEnabled || distillInterval === DEFAULTS.distillInterval} onClick=${() => act(`/memory distill interval ${DEFAULTS.distillInterval}`)}><${icons.arrowUturnLeft} /></button><button class="modal-btn icon-btn" title="Save" disabled=${busy || !distillIntervalDraft || Number(distillIntervalDraft) === distillInterval} onClick=${() => act(`/memory distill interval ${Number(distillIntervalDraft)}`)}><${icons.check} /></button></form>
				</div>
				<div class="settings-compact-row">
					...
					<form class="settings-inline-form" onSubmit=${(event) => { event.preventDefault(); const value = Number(budgetDraft || budget); if (Number.isInteger(value) && value >= 256) act(`/memory budget ${value}`); }}><input aria-label="Memory prompt token budget" type="text" inputmode="numeric" pattern="[0-9]*" value=${budgetDraft} disabled=${busy} onInput=${(event) => setBudgetDraft(event.target.value.replace(/[^0-9]/g, ""))} /><button class="modal-btn icon-btn" title="Reset to ${DEFAULTS.budget}" disabled=${busy || budget === DEFAULTS.budget} onClick=${() => act(`/memory budget ${DEFAULTS.budget}`)}><${icons.arrowUturnLeft} /></button><button class="modal-btn icon-btn" title="Save" disabled=${busy || !budgetDraft || Number(budgetDraft) === budget} onClick=${() => act(`/memory budget ${Number(budgetDraft)}`)}><${icons.check} /></button></form>
				</div>
				<div class="settings-compact-row">
					...
					<form class="settings-inline-form" onSubmit=${(event) => { event.preventDefault(); const value = Number(floorDraft || floor); if (value >= 0 && value <= 1) act(`/memory floor ${value}`); }}><input aria-label="Memory search score floor" type="text" inputmode="decimal" pattern="[0-9.]*" value=${floorDraft} disabled=${busy} onInput=${(event) => setFloorDraft(event.target.value.replace(/[^0-9.]/g, "").replace(/(\..*)\./g, "$1"))} /><button class="modal-btn icon-btn" title="Reset to ${DEFAULTS.floor}" disabled=${busy || floor === DEFAULTS.floor} onClick=${() => act(`/memory floor ${DEFAULTS.floor}`)}><${icons.arrowUturnLeft} /></button><button class="modal-btn icon-btn" title="Save" disabled=${busy || !floorDraft || Number(floorDraft) === floor} onClick=${() => act(`/memory floor ${Number(floorDraft)}`)}><${icons.check} /></button></form>
				</div>
				<div class="settings-compact-row">
