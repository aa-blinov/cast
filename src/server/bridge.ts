...
// for the existing call sites (test/web-bridge.test.ts destructures
// toDisplayMessages; server.ts imports reconcileActiveStream), and
...
// the regexes stay importable from one place.
import { CODE_FENCE_CLOSE_RE, CODE_FENCE_OPEN_RE, parseEvolveJson } from "./bridge/parsers.ts";

export {
	type DisplayMessage,
	type DisplayStreamBlock,
...
export function createServerBridge(result: StartupResult): ServerBridge {
	...
	async function executeCommand(
		...
		// for commands not yet registered. The closure-side helpers may be
