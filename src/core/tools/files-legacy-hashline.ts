/**
 * @deprecated Not wired into the active tool executor (see tools.ts) —
 * kept for reference and as an easy revert path. This is cast's original
 * `read`/`write`/`edit` implementation: hashline-anchored `read`, and an
 ...
 * last read), where fuzzy text matching can silently match the wrong
 * block if the file changed in a way that still clears the similarity
 * threshold. If that trade-off turns out to matter in practice, reverting
 * is a matter of pointing tools.ts back at this file's exports and
 * restoring the old tool schema/prompt.
 */
