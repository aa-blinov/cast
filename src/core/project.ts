...
export function formatSystemEnvironmentBlock(cwd: string, options?: SystemEnvironmentOptions): string {
	...
	if (options.subagent) {
		...
		if (options.mode === "plan") {
			...
			lines.push(
				"- Mode: build — full toolset. Plan mode is initiated only by the user with /plan; it is for establishing task scope and an execution plan before implementation. Do not switch modes, request a mode transition, or write an implementation plan in build mode. Handle the task directly.",
			);
		}
