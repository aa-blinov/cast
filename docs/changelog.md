...
- **The scrollback no longer disappears while you type a long prompt or queue a message mid-turn.** Only the streaming answer was height-bounded; the composer draft and the `[Queued: …]` receipts wrapped to as many rows as they wanted, and once the live region is taller than the terminal Ink clears the screen *and* the scrollback on every frame — 13 full clears for one 600-character draft, 210 (1.8MB of output) for two long queued messages during a stream. The composer now windows a long draft horizontally like readline, with `\u2039`/`\u203a` markers at the clipped edges, and queue/steer receipts are one truncated row each, at most three plus a count. `npm run e2e:tui` covers both.
