...
- **"Load more" in the sidebar looked like it ignored the click.** The button had no pending state at all: the next page of sessions appeared 200ms later on a fast link and well past half a second on a phone, with nothing happening in between. It now shows a spinner and refuses a second click while the page is on its way — measured at 27ms from the click, whatever the latency.
