...
- **`you` and `agent` line their text up.** Both labels are drawn in one field now, so consecutive turns start their prose in the same column instead of two ragged ones (`you` is two cells shorter than `agent`). `reasoning` keeps its own wider field: padding every turn out to match it would spend a ninth of an 80-column terminal on chrome.
