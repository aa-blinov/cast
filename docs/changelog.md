...
- `cast -c` / `cast --continue` is now scoped to the current working directory. Previously it resumed the most recent session *globally* (whatever project was last touched on the machine), which silently dragged the agent into an unrelated project when the user `cd`'d somewhere fresh. It now resumes the most recent session in the *current cwd* and exits 1 with a clear error if no session exists there. `--resume=<id>` still works globally — the lookup by explicit id is not cwd-bound, since the user already knows which session they want. Mirrors `claude -c`'s behaviour (an upstream tracker for the same bug is anthropics/claude-code#35226).
